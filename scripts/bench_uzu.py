#!/usr/bin/env python3
"""Hayabusa Uzu Benchmark: Single Node vs Uzu Bandwidth-First Routing.

Compares throughput of a single Mac Studio vs Uzu-routed cluster (Studio + mini).

Usage:
    python scripts/bench_uzu.py
"""
...
OUTPUT_PATH = SCRIPT_DIR / "bench_uzu.json"
...
def start_server(port: int, slots: int, peers: str | None = None,
                 spillover: float | None = None) -> subprocess.Popen:
    env = os.environ.copy()
...
def print_table(
    ...
    uzu_results: dict[int, BenchResult],
):
    print()
    print("=" * 95)
    print("  Hayabusa Uzu Benchmark")
    print(f"  Single: Mac Studio (llama, {SLOTS} slots)")
    print(f"  Uzu:    Mac Studio (llama) + Mac mini (MLX) — bandwidth-first routing")
    ...
    hdr = (
        "  +------+----------------------------------+----------------------------------+--------+\n"
        "  |      |        Single (Mac Studio)       |     Uzu (Studio + mini)          |        |\n"
        "  | Conc +----------+----------+-------------+----------+----------+------------+ Ratio  |\n"
    ...
    for c in CONCURRENCIES:
        ...
        if not s or not u:
            continue
        ratio = u.agg_tok_per_sec / s.agg_tok_per_sec if s.agg_tok_per_sec > 0 else 0
        print(
            f"  | {c:>4} | {s.agg_tok_per_sec:>8.1f} | {s.avg_latency:>8.0f} | {s.p95:>11.0f} |"
            f" {u.agg_tok_per_sec:>8.1f} | {u.avg_latency:>8.0f} | {u.p95:>10.0f} | {ratio:>5.2f}x |"
        )
...
async def fetch_bandwidth(port: int):
    """Fetch and display Uzu bandwidth stats from cluster status."""
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(f"http://localhost:{port}/v1/cluster/status",
                             timeout=aiohttp.ClientTimeout(total=5)) as resp:
                status = await resp.json()
                bw = status.get("bandwidth", [])
                if bw:
                    print("  Uzu bandwidth stats:")
                    for node in bw:
                        local_tag = " (local)" if node.get("isLocal") else ""
                        print(f"    {node['nodeId']}{local_tag}: "
                              f"{node['ewmaTokPerSec']:.1f} tok/s, "
                              f"{node['activeRequests']} active, "
                              f"{node['totalRequests']} total reqs, "
                              f"{node['totalTokens']} total tok")
                    print()
    except Exception:
        pass
...
def save_results(
    ...
    for c in CONCURRENCIES:
        ...
        rows.append({
            ...
            "uzu": {
                "agg_tok_per_sec": round(u.agg_tok_per_sec, 2),
                "avg_latency_ms": round(u.avg_latency, 1),
                "p50_latency_ms": round(u.p50, 1),
                "p95_latency_ms": round(u.p95, 1),
                "successful": u.successful, "failed": u.failed,
                "wall_time_sec": round(u.wall_time_sec, 3),
                "total_completion_tokens": u.total_completion_tokens,
            },
            "ratio_uzu_over_single": round(ratio, 3),
        })
    ...
    out = {
        ...
        "config": {
            ...
            "cluster": "Mac Studio (llama) + Mac mini (MLX) — Uzu routing",
...
async def main():
    ...
    print("  Hayabusa Uzu Benchmark")
    print(f"  Single: Mac Studio llama ({SLOTS} slots)")
    print(f"  Uzu:    Mac Studio + Mac mini ({MAC_MINI})")
    print(f"  Output={MAX_TOKENS}tok  Samples={SAMPLES_PER_CONC}/conc")
    ...
    print()
    print("-" * 50)
    print("  Phase 2: Uzu Cluster (Mac Studio + Mac mini)")
    print("-" * 50)

    uzu_proc = start_server(LOCAL_PORT, SLOTS, peers=MAC_MINI, spillover=0.8)
    print(f"  PID={uzu_proc.pid}, waiting...")

    if not await wait_for_server(LOCAL_PORT):
        print("ERROR: Uzu server failed to start")
        kill_server(uzu_proc)
        sys.exit(1)
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(f"http://localhost:{LOCAL_PORT}/v1/cluster/status",
                ...
                routing = status.get("routing", "unknown")
                print(f"  Cluster: {len(nodes)} nodes ({len(remote)} remote), routing={routing}")
    except Exception:
        pass
    print()
    ...
    try:
        for conc in CONCURRENCIES:
            r = await run_bench(LOCAL_PORT, "Uzu", conc, SAMPLES_PER_CONC)
            uzu_results[conc] = r

        # Show final bandwidth stats
        await fetch_bandwidth(LOCAL_PORT)
    finally:
        kill_server(uzu_proc)

    # ================================================================
    print_table(single_results, uzu_results)
    save_results(single_results, uzu_results)
